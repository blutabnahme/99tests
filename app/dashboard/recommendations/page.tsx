...
export default function RecommendationsList() {
  ...
  useEffect(() => {
    const handleClickOutside = (e: any) => {
       if (e.target?.closest('.action-menu-trigger')) return;
       setActiveMenu(null);
    };
           ...
           <div className="flex flex-col lg:flex-row gap-3">
              <div className="relative flex-1">
                 <input 
                   ...
                   className="w-full h-10 pl-10 pr-4 text-[13px] bg-gray-50 border border-gray-200 rounded-full focus:outline-none focus:border-primary transition-all placeholder:text-gray-400"
                 />
                    ...
                    return (
                       ...
                       >
                         ...
                         <td className="py-4 text-center relative group/test">
                            ...
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 w-max min-w-[200px] max-w-[280px] bg-white border border-gray-100 shadow-md rounded-lg p-3 z-50 opacity-0 invisible group-hover/test:opacity-100 group-hover/test:visible transition-all duration-150 text-left pointer-events-none">
                         ...
                         <td className="py-4 text-center font-medium text-gray-900" onClick={() => router.push(`/dashboard/recommendations/${row.id}`)} style={{cursor: 'pointer'}}>
                            €{Number(row.test_costs_total || 0).toFixed(2)}
                         </td>
                         ...
                         <td className="py-4 text-right pr-6 relative w-24">
                            ...
                            {activeMenu === row.id && typeof document !== 'undefined' && createPortal(
                               ...
                               >
                                  <button onClick={() => { setActiveMenu(null); router.push(`/dashboard/recommendations/${row.id}`); }} className="text-left px-4 py-2.5 text-[13px] text-gray-700 hover:bg-gray-50 transition-colors font-medium">
                                     View Details
                                  </button>
                                  {row.status === 'created' && (
                                     <>
                                        <button className="text-left px-4 py-2.5 text-[13px] text-gray-700 hover:bg-gray-50 transition-colors font-medium">
                                           Edit Recommendation
                                        </button>
                                        <button className="text-left px-4 py-2.5 text-[13px] text-gray-700 hover:bg-gray-50 transition-colors font-medium">
                                           Send to Patient
                                        ...
                                        <button className="text-left px-4 py-2.5 text-[13px] text-red-600 hover:bg-red-50 transition-colors font-medium">
                                           Delete
                                        </button>
                                     </>
                                  )}
                                  {row.status !== 'created' && (
                                     <button disabled className="text-left px-4 py-2.5 text-[13px] text-gray-400 opacity-50 cursor-not-allowed border-t border-gray-100 bg-gray-50/30">
