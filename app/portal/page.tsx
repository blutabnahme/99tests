...
import {
  ...
  Package, Truck, CheckCircle2, CreditCard, ExternalLink
} from 'lucide-react';
...
export default function PortalHomePage() {
  ...
  const activeOrders = recommendations.filter((o: any) => o.status !== 'cancelled');
  const cancelledOrders = recommendations.filter((o: any) => o.status === 'cancelled');

  const renderOrder = (rec: any) => {
      ...
      >
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            {effectiveStatus === 'kit_shipped' && (
              <div className="text-[11px] text-primary font-medium mt-0.5">Your kit is on the way</div>
            )}
            {effectiveStatus === 'preparing' && (
              <div className="text-[11px] text-gray-400 mt-0.5">Your kit is being prepared</div>
            )}
          </div>
