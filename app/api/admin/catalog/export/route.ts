...
function formatMaterialsForExport(materials: any[] | null): string {
  ...
  return materials.map(m => {
    ...
    if (m.measurement_type === 'volume' && m.required_volume != null) {
      const unit = m.volume_unit || m.unit || 'ml';
      return `${code}:${m.required_volume}${unit}`;
    } else {
      const qty = m.qty ?? 1;
      return `${code}:${qty}`;
    }
...
export async function GET(request: Request) {
 ...
 'Price Zone 1': test.price_zone1 !== null ? test.price_zone1 : '',
 'Price Zone 2': test.price_zone2 !== null ? test.price_zone2 : '',
 'Price Zone 3': test.price_zone3 !== null ? test.price_zone3 : '',
 'Materials': formatMaterialsForExport(test.materials),
 'Sample Shipping': test.sample_shipping || '',
 'Preanalytics': test.preanalytics || '',
 'More Info URL': test.more_info_url || '',
 'EDV Code': test.edv_code || '',
 'GoÄ Digit': test.goae_digit || '',
 'GoÄ Costs': test.goae_costs || '',
 'GoÄ Names': test.goae_names || '',
 'GoÄ Factor': test.goae_factor || '',
