...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: existing } = await supabaseAdmin.from('tt_doctor_test').select('id').eq('doctor_id', doctor_id).eq('test_id', params.id).single();
    if (existing) return NextResponse.json({ error: 'Doctor already has access' }, { status: 409 });

    const { data, error } = await supabaseAdmin
      .from('tt_doctor_test')
      .insert({ doctor_id, test_id: params.id })
      .select('id, doctor_id, created_at, doctor:tt_doctor(id, full_name, email, practice_name)')
      .single();
    if (error) throw error;
    return NextResponse.json(data);
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { error } = await supabaseAdmin.from('tt_doctor_test').delete().eq('doctor_id', doctorId).eq('test_id', params.id);
    if (error) throw error;
