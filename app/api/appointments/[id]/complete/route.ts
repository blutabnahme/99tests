  ...
  try {
    ...
    for (const admin of adminUsers) {
      await createNotification(
        ...
        `Recommendation ${recommendationId.split('-')[1]} requires ${logistics === 'platform' ? 'logistics pickup' : 'lab procession'}.`,
        `/admin/insights` // or admin recommendation view when built
      );
    }
