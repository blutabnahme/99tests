...
export async function POST(
  ...
  try {
    ...
    await createNotification(
      ...
      `Appointment confirmed: ${new Date(scheduled_at).toLocaleDateString()} at ${new Date(scheduled_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} with ${patientName}.`,
      `/bc/dashboard/appointments`
    );
    ...
    if (appData?.case?.doctor_id) {
      await createNotification(
        ...
        'case_update',
        'Appointment Booked',
        `Appointment booked for recommendation ${token.split('-')[1]}: ${new Date(scheduled_at).toLocaleDateString()} at ${new Date(scheduled_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}.`,
