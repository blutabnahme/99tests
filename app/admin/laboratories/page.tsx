...
import { Plus, Edit2, Archive, Loader2, ArchiveRestore, Search, X } from 'lucide-react';
  ...
  const filteredLabs = useMemo(() => {
    return laboratories.filter((lab) => {
      ...
      if (searchQuery) {
        ...
        const matchesOfficial = lab.official_name?.toLowerCase().includes(q);
        const matchesCity = lab.address_city?.toLowerCase().includes(q);
        const matchesAisid = lab.aisid?.toLowerCase().includes(q);
        if (!matchesName && !matchesOfficial && !matchesCity && !matchesAisid) return false;
      }
      // Filter pills
    ...
    <div className="max-w-7xl mx-auto space-y-6 lg:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-[14px]">
            ...
            {!loading && <span className="text-gray-400 ml-1">({filteredLabs.length}{filteredLabs.length !== laboratories.length ? ` of ${laboratories.length}` : ''})</span>}
          </p>
        </div>
        <Button
          variant="primary"
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
        ...
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input
            type="text"
            placeholder="Search by name, city, or AISID..."
            value={searchQuery}
        ...
        <div className="flex flex-wrap items-center gap-2 lg:ml-auto">
          {/* Filter Pills */}
          <button
            onClick={() => setFilterLdt(!filterLdt)}
            className={`h-9 px-3.5 text-[13px] font-medium rounded-full border transition-colors ${filterLdt ? 'bg-blue-100 text-blue-700 border-blue-200' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300'}`}
          >
            LDT
          </button>
          <button
            onClick={() => setFilterPad(!filterPad)}
            className={`h-9 px-3.5 text-[13px] font-medium rounded-full border transition-colors ${filterPad ? 'bg-violet-100 text-violet-700 border-violet-200' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300'}`}
          >
            PAD
          </button>
          <button
            onClick={() => setFilterPrivate(!filterPrivate)}
            className={`h-9 px-3.5 text-[13px] font-medium rounded-full border transition-colors ${filterPrivate ? 'bg-amber-100 text-amber-700 border-amber-200' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300'}`}
          >
            Private
          </button>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                        ...
                        <button
                          ...
                          className={`p-2 rounded-full transition-colors ${lab.is_active ? 'text-gray-400 hover:text-amber-600 hover:bg-amber-50' : 'text-gray-400 hover:text-green-600 hover:bg-green-50'}`}
                ...
                {filteredLabs.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-gray-500 bg-gray-50/50">
                      {laboratories.length === 0 ? 'No laboratories found' : 'No laboratories match your filters'}
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="lg:hidden bg-white border-y border-gray-200">
            {filteredLabs.map((lab, index) => (
              <div
                key={lab.id}
                onClick={() => { setEditingLab(lab); setIsModalOpen(true); }}
                className={`p-4 hover:bg-gray-50/50 active:bg-gray-50 transition-colors ${index !== filteredLabs.length - 1 ? 'border-b border-gray-100' : ''}`}
              >
                ...
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
                  ...
                  <Button
                    variant={lab.is_active ? 'secondary' : 'secondary'}
                    className="flex-1 rounded-full text-[13px] h-9"
