    ...
    <div className="flex-1 min-w-0 w-full font-body print:p-0 print:m-0 print:max-w-full print:bg-white text-near-black">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 print:mb-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center print:hidden w-full sm:w-auto">
          <div className="flex bg-gray-100 p-1 rounded-lg border border-gray-200 overflow-x-auto hide-scrollbar max-w-full -mx-1 px-1 w-full sm:w-auto">
            {['week', 'month', '30d', '90d', 'year', 'all'].map(v => (
              <button 
                ...
                className={`px-4 py-1.5 rounded-md text-[13px] font-semibold whitespace-nowrap shrink-0 transition-all ${dateFilter === v ? 'bg-white text-primary-dark shadow-sm' : 'text-gray-500 hover:text-near-black'}`}
              >
          ...
          <button onClick={exportPDF} className="flex flex-1 sm:flex-none justify-center w-full sm:w-auto items-center gap-2 px-4 py-2 bg-open-bg border border-primary-light text-primary-dark text-[13px] font-semibold rounded-lg hover:bg-primary-light transition-all shrink-0">
            <FileText className="w-4 h-4" /> Export PDF
          </button>
          <button onClick={handleGenerateInvoices} disabled={loading} className="flex flex-1 sm:flex-none justify-center w-full sm:w-auto items-center gap-2 px-4 py-2 bg-primary-dark text-white text-[13px] font-semibold rounded-lg hover:bg-primary transition-all shrink-0 shadow-sm shadow-primary-dark/20">
        ...
        <div className="p-4 sm:p-6 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex bg-gray-100 p-1 rounded-lg border border-gray-200 print:hidden w-full sm:w-auto">
            <button 
              ...
              className={`flex-1 sm:flex-none px-3 py-1 rounded-md text-[12px] font-semibold transition-all ${breakdownView === 'weekly' ? 'bg-white text-near-black shadow-sm' : 'text-gray-500'}`}
            >
              Weekly
            </button>
            <button 
              onClick={() => setBreakdownView('monthly')}
              className={`flex-1 sm:flex-none px-3 py-1 rounded-md text-[12px] font-semibold transition-all ${breakdownView === 'monthly' ? 'bg-white text-near-black shadow-sm' : 'text-gray-500'}`}
            >
              Monthly
            </button>
          </div>
    ...
    <div className={`rounded-2xl border p-3 sm:p-5 shadow-sm flex flex-col justify-between ${highlight ? 'bg-gradient-to-br from-primary to-primary-dark border-transparent text-white' : 'bg-white border-gray-200 print:shadow-none print:border-gray-300'} print:break-inside-avoid`}>
      <div className="flex justify-between items-start mb-3 sm:mb-4">
        <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center shrink-0 border 
          ${highlight ? 'bg-primary/50 border-ruby-400/50' : alert ? 'bg-orange-50 border-orange-100 print:bg-white' : 'bg-gray-50 border-gray-200 print:bg-white'}`}>
          <Icon className={`w-4 h-4 sm:w-5 sm:h-5 ${highlight ? 'text-white' : alert ? 'text-orange-600 print:text-black' : 'text-gray-500 print:text-black'}`} />
        </div>
      </div>
      <div>
        <div className={`font-heading text-[24px] sm:text-[28px] font-medium tracking-tight ${highlight ? 'text-white' : alert ? 'text-orange-600 print:text-black' : className ? className : 'text-near-black'}`}>
          {value}
        </div>
        <div className={`text-[12px] sm:text-[13px] font-medium mt-1 ${highlight ? 'text-primary-light' : 'text-gray-500 print:text-gray-600'}`}>
          {title}
        </div>
      </div>
    </div>
...
function LoadingPlaceholder() {
  return (
    <div className="w-full h-full flex flex-col items-center justify-center text-gray-500">
      <div className="w-6 h-6 rounded-full border-2 border-slate/30 border-t-slate animate-spin mb-2" />
