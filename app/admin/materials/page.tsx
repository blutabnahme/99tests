    ...
    >
      ...
      >
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50/50 rounded-b-[16px] flex items-center justify-end gap-3">
          <Button variant="secondary" onClick={onCancel} className="rounded-full px-5 h-10 text-[14px]" disabled={loading}>
            Cancel
          </Button>
          <button
            onClick={onConfirm}
            disabled={loading}
            className="inline-flex items-center justify-center gap-2 rounded-full px-5 h-10 text-[14px] font-medium bg-red-600 text-white hover:bg-red-700 disabled:opacity-50 transition-colors"
          >
...
export default function MaterialsPage() {
  ...
  }, [activeOnly, searchQuery, tubeTypeFilter, measurementFilter]);

  // ─── Derived: unique tube types for filter dropdown ─────────
  ...
  const filteredMaterials = useMemo(() => {
    return materials.filter((mat) => {
      ...
      if (searchQuery) {
        ...
        const matchesCode = mat.code?.toLowerCase().includes(q);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await fetch('/api/admin/materials/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'delete', ids: deleteConfirm.ids })
      });
      if (!res.ok) {
        const data = await res.json();
  ...
  const activeFilterCount = [
    searchQuery ? 1 : 0,
    tubeTypeFilter !== 'all' ? 1 : 0,
    measurementFilter !== 'all' ? 1 : 0,
  ].reduce((a, b) => a + b, 0);

  const clearFilters = () => {
    setSearchQuery('');
    setTubeTypeFilter('all');
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 lg:space-y-8">
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
        ...
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input
            ...
            className="w-full h-10 pl-10 pr-4 text-[14px] rounded-full border border-gray-200 focus:border-[#008085] focus:ring-1 focus:ring-[#008085] outline-none transition-colors placeholder:text-gray-400"
          />
          {searchQuery && (
            <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 p-0.5 text-gray-400 hover:text-gray-600">
        ...
        <div className="flex flex-wrap items-center gap-3 lg:ml-auto">
          ...
          <select
            ...
            className="h-10 px-4 pr-8 text-[14px] rounded-full border border-gray-200 focus:border-[#008085] focus:ring-1 focus:ring-[#008085] outline-none transition-colors bg-white appearance-none"
          ...
          {activeFilterCount > 0 && (
            <button
              ...
              className="text-[13px] text-gray-500 hover:text-[#008085] underline underline-offset-2 shrink-0"
            >
              Clear filters
            </button>
          )}
      ...
      {someSelected && (
        <div
          className="flex items-center gap-3 bg-[#E6F7F5]/50 border border-[#008085]/20 rounded-[12px] px-4 py-3"
          style={{ animation: 'bulkBarIn 0.2s ease-out' }}
        >
          <style>{bulkBarKeyframes}</style>
          <span className="text-[14px] font-medium text-[#005C5F] whitespace-nowrap">
            {selectedIds.size} selected
          </span>
          <div className="flex-1" />
          <button
            className="inline-flex items-center justify-center rounded-full text-[13px] h-9 px-4 font-medium bg-white border border-gray-200 text-gray-700 hover:border-[#008085] hover:text-[#008085] disabled:opacity-50 transition-colors"
            onClick={() => handleBulkAction('activate')}
            disabled={bulkLoading}
          >
            {bulkLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin mr-1.5" /> : null}
            Activate
          </button>
          <button
            className="inline-flex items-center justify-center rounded-full text-[13px] h-9 px-4 font-medium bg-white border border-gray-200 text-gray-700 hover:border-[#008085] hover:text-[#008085] disabled:opacity-50 transition-colors"
          ...
          <button
            ...
            disabled={bulkLoading}
            className="inline-flex items-center gap-1.5 rounded-full text-[13px] h-9 px-4 font-medium bg-red-600 text-white hover:bg-red-700 disabled:opacity-50 transition-colors"
          >
              ...
              <thead className="bg-gray-50/50 text-gray-500 font-medium text-[12px] uppercase tracking-wider">
                <tr>
                  <th className="px-4 py-4 w-12">
                    <input
                      type="checkbox"
                      checked={allFilteredSelected && filteredMaterials.length > 0}
              ...
              <tbody className="divide-y divide-gray-100 font-body">
                {filteredMaterials.map((mat) => {
                  ...
                  return (
                    <tr
                      key={mat.id}
                      className={`transition-colors cursor-pointer ${isSelected ? 'bg-[#E6F7F5]/30' : 'hover:bg-gray-50/50'}`}
                    ...
                    >
                      ...
                      <td className="px-4 py-4 font-mono text-[13px] text-gray-600">
                        {mat.code}
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                          <button
                            onClick={() => { setEditingMaterial(mat); setIsModalOpen(true); }}
                            className="p-2 text-gray-400 hover:text-[#008085] hover:bg-[#008085]/5 rounded-full transition-colors"
                            title="Edit"
                          >
                          ...
                          <button
                            ...
                            className={`p-2 rounded-full transition-colors ${mat.is_active ? 'text-gray-400 hover:text-amber-600 hover:bg-amber-50' : 'text-gray-400 hover:text-green-600 hover:bg-green-50'}`}
                            title={mat.is_active ? 'Deactivate' : 'Activate'}
                          >
                            {mat.is_active ? <Archive className="w-4 h-4" /> : <ArchiveRestore className="w-4 h-4" />}
                          </button>
                          <button
                            onClick={() => handleSingleDelete(mat.id)}
          ...
          <div className="lg:hidden bg-white border-y border-gray-200">
            ...
            {filteredMaterials.map((mat, index) => {
              ...
              return (
                <div
                  key={mat.id}
                  className={`p-4 transition-colors ${isSelected ? 'bg-[#E6F7F5]/20' : 'hover:bg-gray-50/50 active:bg-gray-50'} ${index !== filteredMaterials.length - 1 ? 'border-b border-gray-100' : ''}`}
                >
                  <div className="flex items-start gap-3">
                    <input
                      ...
                      checked={isSelected}
                      onChange={() => toggleSelect(mat.id)}
                      className="w-4 h-4 mt-1 rounded border-gray-300 text-[#008085] focus:ring-[#008085]"
                    />
                    <div
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
                        ...
                        <Button
                          variant={mat.is_active ? 'secondary' : 'secondary'}
                          className="flex-1 rounded-full text-[13px] h-9"
                        ...
                        <button
                          className="h-9 px-3 rounded-full text-[13px] font-medium text-red-600 border border-red-200 hover:bg-red-50 transition-colors"
                          onClick={(e) => { e.stopPropagation(); handleSingleDelete(mat.id); }}
                        >
                          Delete
                        </button>
                      </div>
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 z-[9998] max-w-md" style={{ animation: 'toastIn 0.25s ease-out' }}>
          ...
          <div className={`flex items-start gap-3 px-4 py-3 rounded-[12px] shadow-lg border ${
