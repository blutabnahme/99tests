...
export function Navbar() {
 ...
 return (
 <>
 <nav className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 px-4 sm:px-6 lg:px-12 h-16 flex items-center justify-between ${navBg ? "bg-white/90 backdrop-blur-md border-b border-gray-200/50" : "bg-transparent border-b border-transparent"}`}>
 ...
 <button className="flex items-center gap-1.5 px-2.5 py-1 hidden sm:flex border border-gray-200 rounded-full bg-transparent text-[13px] font-medium text-gray-500 hover:border-gray-300 hover:text-near-black transition-colors" onClick={() => setNavLangOpen(!navLangOpen)}>
 ...
 <Link href="/login" className="hidden sm:inline-block text-gray-500 text-[14px] font-medium hover:text-near-black transition-colors relative after:absolute after:-bottom-1 after:left-0 after:w-0 after:h-[1.5px] after:bg-primary hover:after:w-full after:transition-colors after:duration-300">
 {t('auth.login')}
 </Link>
 <Link href="/register" className="bg-primary text-white rounded-full px-[18px] sm:px-[22px] py-1.5 sm:py-2 text-[13px] sm:text-[14px] font-semibold hover:bg-primary-dark transition-colors shadow-[0_4px_16px_rgba(0, 128, 133,0.25)] flex items-center">
 {t('auth.register')}
 </Link>
