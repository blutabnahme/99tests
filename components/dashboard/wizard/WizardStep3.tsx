...
import { User, Home, Building, Clock, ChevronRight, ChevronLeft } from 'lucide-react';
...
export default function WizardStep3({ deliveryInfo, setDeliveryInfo, onNext, onBack }: any) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto font-body space-y-6">
            ...
            {COLLECTION_METHODS.map(m => {
              ...
              return (
                <button
                  ...
                  className={`flex items-start gap-4 p-5 rounded-[16px] border-2 text-left transition-all duration-200
        ...
        <section className="mb-10">
          ...
          <div className="flex flex-col gap-3">
             {RESULTS_DELIVERY.map(r => (
               <label key={r.id} className={`flex items-center gap-3 p-4 rounded-[12px] border cursor-pointer transition-all ${deliveryInfo.results === r.id ? 'border-primary bg-primary/5' : 'border-gray-100 hover:bg-gray-50'}`}>
                 <input 
                   type="radio" 
                   name="results" 
                   value={r.id}
      ...
      <div className="flex justify-between items-center mt-4">
         <Button variant="secondary" onClick={onBack} className="rounded-full px-8 h-12 text-[15px] font-semibold flex items-center gap-2 bg-white border border-gray-200 shadow-sm hover:bg-gray-50">
            <ChevronLeft className="w-5 h-5"/> Back
         </Button>
         <Button variant="primary" onClick={onNext} disabled={!isFormValid} className="rounded-full px-8 h-12 text-[15px] font-semibold flex items-center gap-2 shadow-md">
