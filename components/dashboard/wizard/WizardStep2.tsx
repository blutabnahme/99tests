...
export default function WizardStep2({ 
 ...
 if (data.total) {
   let finalData = { ...data };
   if (billingMode === 'doctor' && doctorBillingFeePct != null) {
     const testTotal = data.test_costs_total;
     const serviceFee = testTotal * (doctorBillingFeePct / 100);
     const shippingEstimate = data.shipping_estimate;
     const vatBase = serviceFee + shippingEstimate;
     const vat = vatBase * 0.19;
     const total = testTotal + serviceFee + shippingEstimate + vat;

     finalData.service_fee_pct = doctorBillingFeePct;
     finalData.service_fee = serviceFee;
     finalData.vat = vat;
     finalData.total = total;
   }
   setPricingPreview(finalData);
 }
 } catch (err) {
 console.error(err);
 } finally {
 ...
 }, [stringifiedCart, patient, setPricingPreview, billingMode, doctorBillingFeePct]);
 ...
 className="w-full h-11 pl-11 pr-10 text-[13px] bg-gray-50 border border-gray-200 rounded-full focus:outline-none focus:border-primary transition-colors placeholder:text-gray-400"
 />
 ...
 {tests.map(test => (
 <div key={test.id} className="bg-white rounded-[16px] border border-gray-100 p-4 transition-colors shadow-sm hover:shadow-md">
 ...
 <div className={`bg-white rounded-xl shadow-md overflow-hidden mb-4 flex flex-col transition-colors duration-300 ${cart.length === 0 ? 'h-auto pb-6' : 'max-h-[calc(100vh-120px)] overflow-y-auto'}`}>
 ...
 <div className="flex justify-between text-gray-600">
 <span>Service fee ({pricingPreview.service_fee_pct}%){billingMode === 'doctor' ? ' — Doctor billing' : ''}:</span>
