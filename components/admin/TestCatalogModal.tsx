...
import { X, Loader2, Plus, Trash2, ChevronDown, ChevronRight, Search, Minus, Lock, UserPlus } from 'lucide-react';
...
interface SearchableSelectProps {
 ...
 options: { id: string; name: string; code: string; tube_color?: string; default_volume?: number | null; default_unit?: string; measurement_type?: string }[];
 placeholder?: string;
}
...
interface LabSearchableSelectProps {
  value: string;
  onChange: (value: string) => void;
...
function LabSearchableSelect({ value, onChange, options, placeholder = "Select Laboratory" }: LabSearchableSelectProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [search, setSearch] = useState('');
  const dropdownRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <button
        type="button"
        onClick={() => setIsOpen(!isOpen)}
        className={`w-full text-left text-[14px] h-11 px-4 rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none flex items-center justify-between transition-colors bg-white ${selectedOpt ? 'text-gray-900' : 'text-gray-400'}`}
      >
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-md max-h-60 flex flex-col z-50 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto py-1 max-h-48">
            <div
              ...
              className={`px-3 py-2 hover:bg-gray-50 cursor-pointer text-[13px] transition-colors ${!value ? 'bg-primary/5 text-primary font-medium' : 'text-gray-500 italic'}`}
            >
              -- No Laboratory --
            </div>
            {filteredOptions.length === 0 ? (
              ...
              filteredOptions.map(opt => (
                <div
                  ...
                  className={`px-3 py-2 hover:bg-gray-50 cursor-pointer text-[13px] text-gray-900 transition-colors ${value === opt.id ? 'bg-primary/5 text-primary font-medium' : ''}`}
                >
                  {opt.name}
                </div>
              ))
...
export default function TestCatalogModal({ test, categories, laboratories, onClose, onSuccess }: TestCatalogModalProps) {
 ...
 setMaterials(test.materials.map((m: any) => ({
 ...m,
 measurement_type: m.measurement_type || 'quantity',
 required_volume: m.required_volume ?? null,
 volume_unit: m.volume_unit || m.unit || 'ml',
 })));
 }
 }
 }, [test]);

 // Load private test state
 useEffect(() => {
   if (test) {
     setIsPrivate(test.is_private === true);
   }
 }, [test]);

 // Fetch linked doctors for private test
 useEffect(() => {
   if (test && isPrivate) {
     fetch(`/api/admin/catalog/${test.id}/doctors`)
       .then(r => r.json())
 ...
 useEffect(() => {
   if (!doctorSearch || doctorSearch.length < 2) { setDoctorResults([]); return; }
 ...
 const handleAddDoctor = async (doctorId: string) => {
   ...
   try {
     const res = await fetch(`/api/admin/catalog/${test.id}/doctors`, {
     ...
     if (res.ok) {
       ...
       setDoctorSearch(''); setDoctorResults([]);
     }
   } catch {}
 ...
 const handleRemoveDoctor = async (doctorId: string) => {
   if (!test) return;
   try {
     await fetch(`/api/admin/catalog/${test.id}/doctors?doctor_id=${doctorId}`, { method: 'DELETE' });
 ...
 // Block duplicate: check if this material_id is already used in another row
 const isDuplicate = materials.some((m, i) => i !== index && m.material_id === matId);
 if (isDuplicate) {
 alert('This material is already added. Adjust the volume/quantity on the existing entry instead.');
 return;
 }
 ...
 measurement_type: selected.measurement_type || 'quantity',
 volume: selected.default_volume ?? null,
 unit: selected.default_unit || 'ml',
 required_volume: selected.measurement_type === 'volume' ? null : null,
 volume_unit: selected.default_unit || 'ml',
 qty: selected.measurement_type === 'quantity' ? 1 : 1,
 };
 setMaterials(newMats);
 };

 const updateMaterial = (index: number, field: string, value: string | number | boolean | null) => {
  const newMats = [...materials];
 ...
 required_volume: m.required_volume ? parseFloat(String(m.required_volume)) : undefined,
 ...
 className={`flex items-center gap-2 h-9 px-4 rounded-full text-[13px] font-medium border transition-colors ${formData.is_active ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-gray-100 border-gray-200 text-gray-500'}`}
 >
 <div className={`w-2 h-2 rounded-full ${formData.is_active ? 'bg-emerald-500' : 'bg-gray-400'}`} />
 ...
 <input type="checkbox" checked={isPrivate} onChange={(e) => setIsPrivate(e.target.checked)} className="w-5 h-5 rounded border-gray-300 text-[#008085] focus:ring-[#008085]" />
 <div className="flex items-center gap-2">
 <Lock className="w-4 h-4 text-gray-500" />
 <div>
 <span className="text-[14px] font-medium text-near-black">Private Test</span>
 <p className="text-[12px] text-gray-500">Only assigned doctors can see this test in their catalog</p>
 </div>
 </div>
 </label>
 ...
 <input type="text" autoComplete="off" placeholder="Search doctors by name or email..." value={doctorSearch} onChange={(e) => setDoctorSearch(e.target.value)} className="w-full h-10 pl-10 pr-10 text-[14px] rounded-full border border-gray-200 focus:border-[#008085] focus:ring-1 focus:ring-[#008085] outline-none transition-colors placeholder:text-gray-400" />
 ...
 <button key={doc.id} type="button" onClick={() => handleAddDoctor(doc.id)} disabled={addingDoctor} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors text-left border-b border-gray-100 last:border-b-0">
 ...
 <label className="text-[12px] font-medium text-gray-500 uppercase tracking-wider">Authorized Doctors ({linkedDoctors.length})</label>
 ...
 <button type="button" onClick={() => handleRemoveDoctor(entry.doctor_id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-full transition-colors shrink-0" title="Remove access">
 ...
 <input required name="sku" value={formData.sku} onChange={handleChange} disabled={isEditing} className={`w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400 ${isEditing ? 'bg-gray-50 text-gray-500 cursor-not-allowed' : ''}`} placeholder="e.g. TSH-001" />
 </div>
 
 <div className="space-y-1.5">
 <label className="text-[13px] font-medium text-gray-700">Type *</label>
 <select name="type" required value={formData.type} onChange={handleChange} className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors bg-white appearance-none pr-10 bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20width%3D%2220%22%20height%3D%2220%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2020%2020%22%20fill%3D%22currentColor%22%3E%3Cpath%20fill-rule%3D%22evenodd%22%20d%3D%22M5.293%207.293a1%201%200%20011.414%200L10%2010.586l3.293-3.293a1%201%200%20111.414%201.414l-4%204a1%201%200%2001-1.414%200l-4-4a1%201%200%20010-1.414z%22%20clip-rule%3D%22evenodd%22%20%2F%3E%3C%2Fsvg%3E')] bg-[length:20px_20px] bg-no-repeat bg-[position:right_12px_center]">
 ...
 <input required name="name" value={formData.name} onChange={handleChange} className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
 ...
 <input name={lang} value={(nameTranslations as any)[lang]} onChange={handleTranslationChange} className="w-full h-9 px-3 text-[13px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors" />
 </div>
 ))}
 </div>
 )}
 ...
 <input name="category" list="category-options" value={formData.category} onChange={handleChange} className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" placeholder="Type or select..." />
 ...
 onChange={(val) => setFormData(prev => ({ ...prev, lab_id: val }))}
 ...
 <input type="number" step="0.01" name="lab_cost" value={formData.lab_cost} onChange={handleChange} className="w-full h-11 pl-8 pr-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
 </div>
 ...
 <div className="relative">
 <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">€</span>
 <input type="number" step="0.01" name="price_insured" value={formData.price_insured} onChange={handleChange} className="w-full h-11 pl-8 pr-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
 </div>
 ...
 <div className="relative">
 <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">€</span>
 <input type="number" step="0.01" name="price_uninsured" value={formData.price_uninsured} onChange={handleChange} className="w-full h-11 pl-8 pr-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
 </div>
 ...
 <div className="relative">
 <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">€</span>
 <input type="number" step="0.01" name="price_zone1" value={formData.price_zone1} onChange={handleChange} className="w-full h-11 pl-8 pr-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
 </div>
 ...
 <div className="relative">
 <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">€</span>
 <input type="number" step="0.01" name="price_zone2" value={formData.price_zone2} onChange={handleChange} className="w-full h-11 pl-8 pr-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
 </div>
 ...
 <div className="relative">
 <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">€</span>
 <input type="number" step="0.01" name="price_zone3" value={formData.price_zone3} onChange={handleChange} className="w-full h-11 pl-8 pr-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
 </div>
 </div>
 </div>
 ...
 {materials.map((mat, i) => {
  const isVolumeBased = mat.measurement_type === 'volume';
  const availableOptions = catalogMaterials.filter(cm => 
  cm.id === mat.material_id || 
  !materials.some(existing => existing.material_id === cm.id)
  );

  return (
  <div key={i} className="bg-gray-50 rounded-[12px] px-3 py-3 border border-gray-100">
  <div className="flex items-center gap-2">
  {/* Material Selector — takes remaining space */}
  <div className="flex-1 min-w-0">
  <SearchableSelect
  ...
  {/* Volume or Quantity input — only when material is selected */}
  {mat.material_id && isVolumeBased && (
  <div className="flex items-center gap-1.5 shrink-0">
  <input
  type="number"
  step="0.1"
  min="0"
  value={mat.required_volume ?? ''}
  onChange={e => updateMaterial(i, 'required_volume', e.target.value === '' ? null : e.target.value)}
  placeholder="Vol."
  className="w-20 h-10 px-2.5 text-[13px] text-center rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400 bg-white"
  />
  <span className="text-[12px] text-gray-500 w-6 shrink-0">
  {mat.volume_unit || 'ml'}
  </span>
  {mat.volume != null && (
  <span className="text-[11px] text-gray-400 shrink-0 whitespace-nowrap">
  / {mat.volume} {mat.unit || 'ml'}
  </span>
  )}
  </div>
  )}
  ...
  {mat.material_id && !isVolumeBased && (
  <div className="flex items-center gap-1 border border-gray-200 rounded-full bg-white h-10 px-1.5 shadow-sm shrink-0">
  <button type="button" onClick={() => updateMaterial(i, 'qty', Math.max(1, (mat.qty || 1) - 1))} className="w-7 h-7 flex items-center justify-center text-gray-400 hover:text-near-black rounded-full hover:bg-gray-50 transition-colors"><Minus className="w-3.5 h-3.5" /></button>
  <span className="w-6 text-center text-[13px] font-medium text-near-black">{mat.qty || 1}</span>
  <button type="button" onClick={() => updateMaterial(i, 'qty', (mat.qty || 1) + 1)} className="w-7 h-7 flex items-center justify-center text-gray-400 hover:text-near-black rounded-full hover:bg-gray-50 transition-colors"><Plus className="w-3.5 h-3.5" /></button>
  </div>
  )}
 ...
 <select name="sample_shipping" value={formData.sample_shipping} onChange={handleChange} className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors bg-white appearance-none pr-10 bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20width%3D%2220%22%20height%3D%2220%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2020%2020%22%20fill%3D%22currentColor%22%3E%3Cpath%20fill-rule%3D%22evenodd%22%20d%3D%22M5.293%207.293a1%201%200%20011.414%200L10%2010.586l3.293-3.293a1%201%200%20111.414%201.414l-4%204a1%201%200%2001-1.414%200l-4-4a1%201%200%20010-1.414z%22%20clip-rule%3D%22evenodd%22%20%2F%3E%3C%2Fsvg%3E')] bg-[length:20px_20px] bg-no-repeat bg-[position:right_12px_center]">
 ...
 <textarea name="preanalytics" value={formData.preanalytics} onChange={handleChange} className="w-full min-h-[80px] p-4 text-[14px] rounded-[16px] border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>

 <div className="space-y-1.5 md:col-span-2">
 <label className="text-[13px] font-medium text-gray-700">More Info URL</label>
 <input type="url" name="more_info_url" value={formData.more_info_url} onChange={handleChange} className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" placeholder="https://" />
 </div>
 </div>
 ...
 <input name="edv_code" value={formData.edv_code} onChange={handleChange} className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
 <div className="space-y-1.5">
 <label className="text-[13px] font-medium text-gray-700">GoÄ Digit(s)</label>
 <input name="goae_digit" value={formData.goae_digit} onChange={handleChange} placeholder="e.g. 4288 (2x)" className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
 <div className="space-y-1.5">
 <label className="text-[13px] font-medium text-gray-700">GoÄ Factor</label>
 <input name="goae_factor" value={formData.goae_factor} onChange={handleChange} placeholder="e.g. 1.15" className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
 <div className="space-y-1.5 md:col-span-2">
 <label className="text-[13px] font-medium text-gray-700">GoÄ Costs</label>
 <input name="goae_costs" value={formData.goae_costs} onChange={handleChange} placeholder="e.g. 20.40 (2x)" className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
 <div className="space-y-1.5 md:col-span-2">
 <label className="text-[13px] font-medium text-gray-700">GoÄ Names (Description)</label>
 <textarea name="goae_names" value={formData.goae_names} onChange={handleChange} className="w-full min-h-[80px] p-4 text-[14px] rounded-[16px] border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
 </div>
 </div>
 ...
 className="w-full pl-10 pr-4 h-11 rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors text-[14px]"
 />
