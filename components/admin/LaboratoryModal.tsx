...
export default function LaboratoryModal({ lab, onClose, onSuccess }: LaboratoryModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    address_street: '', address_zip: '', address_city: '', address_country: 'D',
    contact_email: '', contact_phone: '', aisid: '', customer_number: ''
  });

  const [ldtEnabled, setLdtEnabled] = useState(false);
  const [ldtConfig, setLdtConfig] = useState({
    einsender_id: '', customer_number: '', charset: '4', version: 'LDT4.20',
    profile_category_label: '', filename_prefix: ''
  });
  ...
  useEffect(() => {
    if (lab) {
      setFormData({
        name: lab.name || '', official_name: lab.official_name || '',
        practice_name: lab.practice_name || '', slug: lab.slug || '',
        address_street: lab.address_street || '', address_zip: lab.address_zip || '',
        address_city: lab.address_city || '', address_country: lab.address_country || 'D',
        contact_email: lab.contact_email || '', contact_phone: lab.contact_phone || '',
        aisid: lab.aisid || '', customer_number: lab.customer_number || ''
      });
      if (lab.ldt_config) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); throw new Error(data.error || 'Failed to save laboratory'); }
      onSuccess();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-0" style={{ backgroundColor: 'rgba(26, 29, 35, 0.5)' }} onClick={onClose}>
      <div className="bg-white rounded-[16px] shadow-xl w-full max-w-2xl max-h-[85vh] overflow-y-auto flex flex-col mx-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 shrink-0">
          <h2 className="font-heading font-medium text-[20px] text-near-black">{isEditing ? 'Edit Laboratory' : 'Add Laboratory'}</h2>
          <button onClick={onClose} className="p-2 text-gray-400 hover:text-near-black hover:bg-gray-100 rounded-full transition-colors"><X className="w-5 h-5" /></button>
        </div>
          ...
          <form id="lab-form" onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input required name="name" value={formData.name} onChange={handleChange} className={inputClasses} placeholder="e.g. MVZ Labor München" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[13px] font-medium text-gray-700">Official Name</label>
                <input name="official_name" value={formData.official_name} onChange={handleChange} className={inputClasses} />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <input name="slug" value={formData.slug} onChange={handleChange} className={inputClasses} placeholder="Auto-generated if empty" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[13px] font-medium text-gray-700">Street</label>
                <input name="address_street" value={formData.address_street} onChange={handleChange} className={inputClasses} />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <input name="address_country" value={formData.address_country} onChange={handleChange} className={inputClasses} />
              </div>
              <div className="space-y-1.5">
                <label className="text-[13px] font-medium text-gray-700">Contact Email</label>
                <input type="email" name="contact_email" value={formData.contact_email} onChange={handleChange} className={inputClasses} />
              </div>
              <div className="space-y-1.5">
                <label className="text-[13px] font-medium text-gray-700">Contact Phone</label>
                <input type="tel" name="contact_phone" value={formData.contact_phone} onChange={handleChange} className={inputClasses} />
              </div>
              <div className="space-y-1.5">
                <label className="text-[13px] font-medium text-gray-700">AISID</label>
                <input name="aisid" value={formData.aisid} onChange={handleChange} className={inputClasses} />
              </div>
              <div className="space-y-1.5">
                <label className="text-[13px] font-medium text-gray-700">Customer Number</label>
                <input name="customer_number" value={formData.customer_number} onChange={handleChange} className={inputClasses} />
              </div>
            </div>
              ...
              {showLdt && (
                <div className="p-4 border-t border-gray-200 space-y-4">
                  <label className="flex items-center gap-3 cursor-pointer" onClick={(e) => e.stopPropagation()}>
                    <input type="checkbox" checked={ldtEnabled} onChange={(e) => setLdtEnabled(e.target.checked)} className="w-5 h-5 rounded border-gray-300 text-[#008085] focus:ring-[#008085]" />
                    <div>
                  ...
                  {ldtEnabled && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                      <div className="space-y-1.5">
                        ...
                        <input name="einsender_id" value={ldtConfig.einsender_id} onChange={handleLdtChange} className={inputClasses} />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[13px] font-medium text-gray-700">Customer Number (FK 8312)</label>
                        <input name="customer_number" value={ldtConfig.customer_number} onChange={handleLdtChange} className={inputClasses} placeholder="e.g. 25997" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[13px] font-medium text-gray-700">Charset</label>
                        <select name="charset" value={ldtConfig.charset} onChange={handleLdtChange} className={selectClasses} style={selectBg}>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input name="version" value={ldtConfig.version} onChange={handleLdtChange} className={inputClasses} />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[13px] font-medium text-gray-700">Profile Category Label</label>
                        <input name="profile_category_label" value={ldtConfig.profile_category_label} onChange={handleLdtChange} className={inputClasses} />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[13px] font-medium text-gray-700">Filename Prefix</label>
                        <input name="filename_prefix" value={ldtConfig.filename_prefix} onChange={handleLdtChange} className={inputClasses} />
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="border border-gray-200 rounded-[12px] overflow-hidden">
              ...
              {showPad && (
                <div className="p-4 border-t border-gray-200">
                  <label className="flex items-center gap-3 cursor-pointer" onClick={(e) => e.stopPropagation()}>
                    <input type="checkbox" checked={padEnabled} onChange={(e) => setPadEnabled(e.target.checked)} className="w-5 h-5 rounded border-gray-300 text-[#008085] focus:ring-[#008085]" />
                    <div>
