...
export default function MaterialModal({ material, onClose, onSuccess }: MaterialModalProps) {
 ...
 tube_type: '',
 tube_color: '',
 measurement_type: 'quantity' as 'volume' | 'quantity',
 default_volume: '',
 default_unit: 'ml',
 sort_order: '0',
 is_active: true,
 ...
 measurement_type: material.measurement_type || 'quantity',
 ...
 <input required name="code" value={formData.code} onChange={handleChange} className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" placeholder="e.g. EDTA-S" />
 </div>
 
 <div className="space-y-1.5">
 <label className="text-[13px] font-medium text-gray-700">Name *</label>
 <input required name="name" value={formData.name} onChange={handleChange} className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" placeholder="e.g. EDTA Röhrchen" />
 </div>

 <div className="space-y-1.5 md:col-span-2">
 <label className="text-[13px] font-medium text-gray-700">Description</label>
 <textarea name="description" value={formData.description} onChange={handleChange} className="w-full min-h-[80px] p-4 text-[14px] rounded-[16px] border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>

 <div className="space-y-1.5 md:col-span-2">
  <label className="text-[13px] font-medium text-gray-700">Measurement Type *</label>
  <p className="text-[12px] text-gray-500 mb-2">
  How is this material measured when linked to a test?
  </p>
  <div className="flex gap-2">
  <button
  type="button"
  onClick={() => setFormData(prev => ({ ...prev, measurement_type: 'volume' }))}
  className={`flex-1 h-11 rounded-full text-[14px] font-medium border transition-colors ${
  formData.measurement_type === 'volume'
  ...
  <button
  type="button"
  onClick={() => setFormData(prev => ({ ...prev, measurement_type: 'quantity' }))}
  className={`flex-1 h-11 rounded-full text-[14px] font-medium border transition-colors ${
  formData.measurement_type === 'quantity'
  ? 'bg-primary text-white border-primary'
  : 'bg-white text-gray-700 border-gray-200 hover:border-primary hover:text-primary'
  }`}
  >
 ...
 <input name="tube_type" value={formData.tube_type} onChange={handleChange} className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" placeholder="e.g. Röhrchen, Abstrich" />
 </div>
 ...
 className={`w-full h-11 ${formData.tube_color ? 'pl-10' : 'pl-4'} pr-10 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors bg-white appearance-none`}
 ...
 {formData.measurement_type === 'volume' && (
  <>
  <div className="space-y-1.5">
  <label className="text-[13px] font-medium text-gray-700">Tube Capacity</label>
  <input type="number" step="0.1" name="default_volume" value={formData.default_volume} onChange={handleChange} className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" placeholder="e.g. 7.5" />
  </div>

  <div className="space-y-1.5">
  <label className="text-[13px] font-medium text-gray-700">Unit</label>
  <select name="default_unit" value={formData.default_unit} onChange={handleChange} className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors bg-white appearance-none pr-10" style={{ backgroundImage: `url("data:image/svg+xml;charset=US-ASCII,%3Csvg%20width%3D%2220%22%20height%3D%2220%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2020%2020%22%20fill%3D%22currentColor%22%3E%3Cpath%20fill-rule%3D%22evenodd%22%20d%3D%22M5.293%207.293a1%201%200%20011.414%200L10%2010.586l3.293-3.293a1%201%200%20111.414%201.414l-4%204a1%201%200%2001-1.414%200l-4-4a1%201%200%20010-1.414z%22%20clip-rule%3D%22evenodd%22%20%2F%3E%3C%2Fsvg%3E")`, backgroundRepeat: 'no-repeat', backgroundPosition: 'right 12px center', backgroundSize: '20px 20px' }}>
 ...
 <input type="number" name="sort_order" value={formData.sort_order} onChange={handleChange} className="w-full h-11 px-4 text-[14px] rounded-full border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors placeholder:text-gray-400" />
 </div>
